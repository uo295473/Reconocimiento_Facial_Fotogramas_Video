...
MODEL_PATH     = "/media/nas/carmengcamp/TFG/models/arcface/w600k_r50.onnx" #ruta del modelo
...
try:
    ...
    print(f"Modelo       : w600k_r50 (ArcFace ResNet50)")
    ...
    for person_entry in tqdm(os.scandir(dataset_path), desc="Procesando personas"): #recorre cada persona del dataset
        ...
        for img_entry in os.scandir(person_entry.path): #recorre cada imagen de esa persona
            ...
            try:
                ...
                row_data["Embedding_ArcFace"]= embedding.tolist() #guarda el embedding como lista
                row_data["Tiempo_ArcFace"]= elapsed
            except Exception as e: #si falla, se guarda None
                print(f"Error con {img_path}: {e}")
                row_data["Embedding_ArcFace"]= None
                row_data["Tiempo_ArcFace"]= None
            data.append(row_data)

    df = pd.DataFrame(data)
    pkl_embeddings = f"/media/nas/carmengcamp/TFG/ArcFace/embeddings_arcface_{dataset_name}.pkl"
    df.to_pickle(pkl_embeddings) #guarda los embeddings en un .pkl

    count = df["Embedding_ArcFace"].notna().sum()
    print(f"Embeddings generados para ArcFace: {count}")

    columnas   = {"Embedding_ArcFace": "Tiempo_ArcFace"}
    salida     = f"/media/nas/carmengcamp/TFG/ArcFace/resultados_arcface_{dataset_name}.csv"
